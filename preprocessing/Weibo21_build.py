...
def generate_ambiguity():
    ...
    xlsxs = [
        f"./train_datasets.xlsx",
        f"./test_datasets.xlsx",
    ]
    for xlsx in xlsxs:
        ...
        while (
            (num_real_count < THRESH or num_fake_count < THRESH)
            and len(valid_list_negative_text) > 0
            and len(valid_list_positive) > 0
        ):  # title label content image B C E F
            ...
            for new_image in new_images:
                is_found_image = cv2.imread(f"/home/yifan40/MAB_OFND/data/weibo/" + new_image)
                if is_found_image is not None:
                    image_width, image_height = (
                        is_found_image.shape[0],
                        is_found_image.shape[1],
                    )
    ...
    ambiguity_excel = "./{}_{}.xlsx".format(
        dataset_name, dataset_name, "train_ambiguity_new"
    )
...
def download_images_and_load():
    ...
    json_files = [
        "/home/yifan40/MAB_OFND/data/weibo/fake_release_all.json",
        "/home/yifan40/MAB_OFND/data/weibo/real_release_all.json",
    ]
    folders = [
        "/home/yifan40/MAB_OFND/data/weibo/rumor_images/",
        "/home/yifan40/MAB_OFND/data/weibo/nonrumor_images/",
    ]
    ...
    for i in range(2):
        ...
        while line:
            try:
                ...
                image_name = ""
                if isinstance(piclists, float):
                    piclists = []
                if not isinstance(piclists, list):
                    piclists = [piclists]
                for full_image_name in piclists:
                    if full_image_name[:2] == "//":
                        full_image_name = "http:" + full_image_name
                        ...
                        if short_name not in images_set:
                            if conduct_download:
                                try:
                                    ...
                                    image_name = (
                                        image_name + "rumor_images/" + short_name + "|"
                                    )
    ...
    df.to_excel(
        "./all_datasets_Weibo21_origin.xlsx"
    )
...
def reload_Weibo21():
            ...
            if image_names is not None and len(image_names) != 0:
                ...
                for image_name in image_name_list:
                    ...
                    if short_name in images_set:
                        ...
                        is_found_image = (
                            cv2.imread("/home/yifan40/MAB_OFND/data/weibo/" + new_image) is not None
    ...
    filename = "./train_datasets.xlsx"
    df = pd.DataFrame(train_list)
    df.to_excel(filename)
    print(f"File saved at {filename}")
    filename = "./val_datasets.xlsx"
    df = pd.DataFrame(val_list)
    df.to_excel(filename)
    print(f"File saved at {filename}")
    filename = "./test_datasets.xlsx"
    df = pd.DataFrame(test_list)
    df.to_excel(filename)
    print(f"File saved at {filename}")
    print(
        "length {} CV_Error {} No_valid {}".format(
        )
    )
    print(
        "Train_set {} Test_set {} Val_set {}".format(
            len(train_list), len(test_list), len(val_list)
        )
    )
